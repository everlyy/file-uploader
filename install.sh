...
client_script_destination=/usr/bin
...
sudo cp $client_upload_script $client_script_destination/upload-file

read -p "Do you want to install the context menu entry for Dolphin? " -n 1 -r
echo
if [[ $REPLY =~ ^[Yy]$ ]]
then
